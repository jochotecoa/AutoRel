...
# Set file locations				
sampledir <- "/share/script/hecatos/juantxo/auto_significant/data/r_odaf/"
outputdir <- paste(sampledir, "Output/", sep="")
if(!dir.exists(outputdir)) {dir.create(outputdir)}
# Names of files to load
SampleDataFile <- "sampleData_21.csv" #This comma delimited file contains the merged RSEM.genes.results files
SampleKeyFile <- "samplekeyTEST_21.csv" #This comma delimited file contains at least 2 columns: NAME (sample names identical to the column names of sampleData) and Compound (needs to identify to which group the sample belongs -> ExperimentalGroup & ControlGroup)

# Specify which groups need to be compared 
Samp4compare<- c("APA") # Experimental group, needs to correspond with the SampleKeyFile. Several condition can be tested c("test1","test2), with the corresponding control
Cont4compare<- c("ConDMSO") # Control group, needs to correspond with the SampleKeyFile
