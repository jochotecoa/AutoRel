source('scripts/functions/functions_JOA.R')
forceLibrary(c('mlbench', 'caret', 'doMC', 'dplyr', 'RANN'))

# source('script/recursive_feature_elimination/load_data_CSimcae.R')
forceLibrary(c('rrcov', 'rrcovHD')) # Needed for bagged trees

apap_data = apap_dataset_path %>% readRDS()
colnames(apap_data)[ncol(apap_data)] = 'relevance'

if (!exists('train_data') | !exists('test_data')) {
  index <- createDataPartition(apap_data$relevance, p = 0.75, list = FALSE)
  train_data <- apap_data[index, ]
  test_data  <- apap_data[-index, ]
}

colnames(train_data) = colnames(train_data) %>% 
  make.names()
colnames(test_data) = colnames(test_data) %>% 
  make.names()


model_CSimca <- caret::train(relevance ~ .,
                             data = train_data,
                             method = "CSimca",
                             preProcess = c("scale", "center"),
                             trControl = trControl)

if (!dir.exists(paste0(train_mod_path, '/CSimca/'))) {
  dir.create(paste0(train_mod_path, '/CSimca/'), recursive = T)
}

model_CSimca %>% saveRDS(paste0(train_mod_path, '/CSimca/original.rds'))


final <- data.frame(actual = test_data$relevance,
                    predict(model_CSimca, newdata = test_data))
final$predict = final[, 2] %>% as.factor()

cm_original <- confusionMatrix(final$predict, test_data$relevance)

if (!dir.exists(paste0(conf_matr_path, '/CSimca/'))) {
  dir.create(paste0(conf_matr_path, '/CSimca/'), recursive = T)
}

# cm_over %>% saveRDS(paste0(conf_matr_path, '/CSimca/over-sampling.rds')
cm_original %>% saveRDS(paste0(conf_matr_path, '/CSimca/original.rds'))
# cm_under %>% saveRDS(paste0(conf_matr_path, '/CSimca/under-sampling.rds')

